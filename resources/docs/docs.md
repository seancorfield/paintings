...
The `home-routes` handler in the `paintings2.routes.home` namespace
...
The `home-page` function will in turn call the `paintings2.layout/render` function
...
The routes are aggregated and wrapped with middleware in the `paintings2.handler` namespace:
...
Request middleware functions are located under the `paintings2.middleware` namespace.

This namespace is reserved for any custom middleware for the application. Some default middleware is
already defined here. The middleware is assembled in the `wrap-base` function.

Middleware used for development is placed in the `paintings2.dev-middleware` namespace found in
